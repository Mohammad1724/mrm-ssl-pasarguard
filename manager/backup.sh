...
create_backup() {
    ...
    local DB_CONTAINER=$(docker ps --format '{{.Names}}' | grep "timescaledb\|postgres" | head -1)
    
    if [ -n "$DB_CONTAINER" ]; then
        # Try dumping (Postgres standard)
    ...
    # 2. Copy Config Files (Safe to copy while running)
