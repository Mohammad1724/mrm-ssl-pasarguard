#!/usr/bin/env bash
#==============================================================================
# MRM Migration Tool - V10.0 (Final Production - PostgreSQL + Full Data Import)
#==============================================================================
...
migrate_postgresql_to_mysql() {
    ...
    # Import each admin
    run_pg "SELECT id, username, hashed_password, COALESCE(is_sudo, false), COALESCE(telegram_id, 0), COALESCE(used_traffic, 0) FROM admins;" | while IFS='|' read -r id username hashed_password is_sudo telegram_id used_traffic; do
        [ -z "$id" ] && continue
    ...
    run_pg "SELECT id, tag FROM inbounds;" | while IFS='|' read -r id tag; do
        [ -z "$id" ] && continue
        run_mysql "INSERT INTO inbounds (id, tag, protocol) VALUES ($id, '$tag', 'mixed');"
    ...
    run_pg "SELECT id, username, COALESCE(status, 'active'), COALESCE(used_traffic, 0), data_limit, expire, COALESCE(admin_id, 1), note, created_at FROM users;" | while IFS='|' read -r id username status used_traffic data_limit expire admin_id note created_at; do
        [ -z "$id" ] && continue
        ...
        [ "$created_at" != "NOW()" ] && created_at="'$created_at'"
        
        # Escape note
        note=$(echo "$note" | sed "s/'/''/g")
        [ -z "$note" ] && note=""
        
        run_mysql "INSERT INTO users (id, username, status, used_traffic, data_limit, expire, admin_id, note, created_at) VALUES ($id, '$username', '$status', $used_traffic, $data_limit, $expire, $admin_id, '$note', $created_at);"
    done
    ...
    local imported_proxies=$(run_mysql "SELECT COUNT(*) FROM proxies;")
    mok "Imported $imported_proxies proxies"
    
    echo ""
    minfo "[5/8] Importing Hosts..."
    ...
    if [ -n "$host_count" ] && [ "$host_count" -gt 0 ]; then
        ...
        mok "Imported $host_count hosts"
    else
        mwarn "No hosts found in source"
    fi
    
    echo ""
    minfo "[6/8] Importing Core Configs..."
    
    local config_count=$(run_pg "SELECT COUNT(*) FROM core_configs;" 2>/dev/null)
    
    if [ -n "$config_count" ] && [ "$config_count" -gt 0 ]; then
        run_pg "SELECT id, name, config FROM core_configs;" | while IFS='|' read -r id name config; do
            [ -z "$id" ] && continue
            
            # Replace paths in config
            config="${config//pasarguard/rebecca}"
            config=$(echo "$config" | sed "s/'/''/g")
            name=$(echo "$name" | sed "s/'/''/g")
            
            run_mysql "INSERT INTO core_configs (id, name, config, created_at) VALUES ($id, '$name', '$config', NOW()) ON DUPLICATE KEY UPDATE config='$config';"
        done
        
        mok "Imported core configs"
    fi
    
    echo ""
    minfo "[7/8] Importing Nodes..."
    
    local node_count=$(run_pg "SELECT COUNT(*) FROM nodes;" 2>/dev/null)
    
    if [ -n "$node_count" ] && [ "$node_count" -gt 0 ]; then
        run_pg "SELECT id, name, address, port, api_port FROM nodes;" | while IFS='|' read -r id name address port api_port; do
            [ -z "$id" ] && continue
...
do_full_migration() {
    migration_init
    clear
    ui_header "MRM MIGRATION TOOL V10.0"
    ...
    if [ -d "/opt/rebecca" ]; then
        ...
        if [ -n "$MYSQL_CONTAINER" ] && [ -n "$DB_PASS" ]; then
            ...
            docker exec "$MYSQL_CONTAINER" mysql -uroot -p"$DB_PASS" rebecca -e "
                ...
                UNION SELECT 'Inbounds', COUNT(*) FROM inbounds;" 2>/dev/null
        fi
    fi
