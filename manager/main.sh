...
tools_menu() {
    while true; do
        ...
        echo "2) Theme Manager (Subscription Page)"
        echo "3) Inbound Wizard (Create Config)"
        echo "4) Edit Panel Config (.env)"
        echo "5) Edit Node Config (.env)"
        echo "6) Restart Panel Service"
        echo "7) Restart Node Service"
        echo "8) Show Node SSL Paths"
        echo "9) Back"
        echo -e "${BLUE}===========================================${NC}"
        read -p "Select: " T_OPT
        case $T_OPT in
            ...
            2) theme_menu ;;
            3) inbound_menu ;;
            4) edit_file "$PANEL_ENV" ;;
            5) edit_file "$NODE_ENV" ;;
            6) restart_service "panel"; pause ;;
            7) restart_service "node"; pause ;;
            8) show_node_ssl ;; # Added requested feature here
            9) return ;;
            *) ;;
        esac
    done
