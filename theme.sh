        ...
        <div class="card">
            <div class="grid">
                <div>
                    ...
                    <div class="btn-grid">
                        <button class="btn btn-primary" onclick="copyToClipboard('{{ subscription_url }}')">کپی لینک</button>
    ...
    <script>
        ...
        function showConfigs() {
            ...
            fetch(window.location.pathname + '/links')
                ...
                .then(function(text) {
                    if (text) {
                        list.innerHTML = '<button class="btn btn-primary" style="height:32px; font-size:12px; margin-bottom:10px; width:100%" onclick="copyToClipboard(\'' + text.replace(/\n/g, '\\n') + '\')">کپی همه</button>';
